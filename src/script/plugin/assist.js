...
function findComponentUpwardByRef(context, ref) {
    if (typeof ref === 'string') {
        let parent = context.$parent;
        let parentRefs = parent.$refs;
        //循环结束条件为到达根节点；找到指定ref vm
        while (parent && !parentRefs[ref]) {
            parent = parent.$parent;
            if (parent) parentRefs = parent.$refs;
        }
        if (parent)
