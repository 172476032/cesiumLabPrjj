...
function createBaseLayer(v, map) {
  ...
  if (v.type == "arcgis-wmts") {
  ...
  } else if (v.type == "arcgis-wms") {
    // arcgis rest 服务(ImageArcGISRest类型，export一整张图片，适合数据量较小的情况)GLL
    baseLayer = new ThematicLayer(
      v.id,
      v.layer_name,
      v.type,
      v.real_name,
    ...
    baseLayer = new BaseLayer( //加载xyz类型的 arcgis rest 服务
      v.id,
      v.layer_name,
      v.type,
      v.real_name,
...
export default {
  createBaseLayer, //创建底图方法
  createThematicLayer //创建专题图方法
};
